...
fn export_and_import_arrow() {
    ...
    let ipc_blob: Vec<u8> = conn.query_row(
        "SELECT vector_export_arrow('emb', 'float4')",
        [],
        |row| row.get(0),
    ).unwrap();
    ...
    conn.execute_batch(
        "CREATE VIRTUAL TABLE emb2 USING vector(dim=3, type=float4, metric=l2)"
    ).unwrap();
    ...
    let count: i64 = conn.query_row(
        "SELECT COUNT(*) FROM emb2", [], |row| row.get(0)
    ).unwrap();
    assert_eq!(count, 2);
}
