...
fn add_and_search_float4() {
    ...
    let results = idx.search(&VectorType::Float4.slice_to_blob(&query), 2).unwrap();
...
fn serialize_round_trip() {
    ...
    let query: Vec<f32> = vec![1.0, 0.0, 0.0];
    let results = idx2.search(&VectorType::Float4.slice_to_blob(&query), 1).unwrap();
...
fn search_empty_index() {
    let idx = HnswIndex::new(3, VectorType::Float4, DistanceMetric::L2, None).unwrap();
    let query: Vec<f32> = vec![1.0, 0.0, 0.0];
    let results = idx.search(&VectorType::Float4.slice_to_blob(&query), 10).unwrap();
