...
/// Run a single (type, metric) combination:
///   1. Create a virtual table with dim=4, the given type and metric.
...
fn run_combination(vec_type: &str, metric: &str) {
    ...
    conn.execute_batch(&insert_sql)
        .unwrap_or_else(|e| panic!("INSERT failed for ({vec_type}, {metric}): {e}"));
    ...
    let mut stmt = conn
        ...
        .unwrap_or_else(|e| panic!("PREPARE failed for ({vec_type}, {metric}): {e}"));

    let rows: Vec<(i64, f64)> = stmt
        .query_map(params![query_blob.as_slice()], |row| {
            Ok((row.get(0)?, row.get(1)?))
        })
        .unwrap_or_else(|e| panic!("query_map failed for ({vec_type}, {metric}): {e}"))
        .collect::<Result<Vec<_>, _>>()
        .unwrap_or_else(|e| panic!("row collect failed for ({vec_type}, {metric}): {e}"));

    assert!(
        !rows.is_empty(),
        "KNN returned no results for ({vec_type}, {metric})"
    );
    assert_eq!(
        rows[0].0, 1,
        "Expected rowid 1 for ({vec_type}, {metric}), got {}",
        rows[0].0
    );
}
...
fn type_float2_metric_l2() {
    run_combination("float2", "l2");
}

#[test]
fn type_float2_metric_cosine() {
    run_combination("float2", "cosine");
}

#[test]
fn type_float2_metric_ip() {
    run_combination("float2", "ip");
}
...
fn type_float4_metric_l2() {
    run_combination("float4", "l2");
}

#[test]
fn type_float4_metric_cosine() {
    run_combination("float4", "cosine");
}

#[test]
fn type_float4_metric_ip() {
    run_combination("float4", "ip");
}
...
fn type_float8_metric_l2() {
    run_combination("float8", "l2");
}

#[test]
fn type_float8_metric_cosine() {
    run_combination("float8", "cosine");
}

#[test]
fn type_float8_metric_ip() {
    run_combination("float8", "ip");
}
...
fn type_int1_metric_l2() {
    run_combination("int1", "l2");
}

#[test]
fn type_int1_metric_cosine() {
    run_combination("int1", "cosine");
}

#[test]
fn type_int1_metric_ip() {
    run_combination("int1", "ip");
}
...
fn type_int2_metric_l2() {
    run_combination("int2", "l2");
}

#[test]
fn type_int2_metric_cosine() {
    run_combination("int2", "cosine");
}

#[test]
fn type_int2_metric_ip() {
    run_combination("int2", "ip");
}
...
fn type_int4_metric_l2() {
    run_combination("int4", "l2");
}

#[test]
fn type_int4_metric_cosine() {
    run_combination("int4", "cosine");
}

#[test]
fn type_int4_metric_ip() {
    run_combination("int4", "ip");
}

/// Smoke test: all 18 combinations are covered by the individual tests above.
/// This test documents the expected matrix shape without re-running them.
#[test]
fn type_matrix_coverage() {
    assert_eq!(TYPES.len(), 6, "expected 6 element types");
    assert_eq!(METRICS.len(), 3, "expected 3 distance metrics");
    assert_eq!(TYPES.len() * METRICS.len(), 18, "expected 18 combinations");
}
